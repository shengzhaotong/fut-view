			...
			<view class="t-login">
				<form class="cl">
					<view class="t-a">
						...
						<input type="number" name="phone" placeholder="请输入账号" maxlength="11" v-model="loginData.id" />
					</view>
					<view class="t-a">
...
export default {
	...
	onShow () {
		...
		if (currPage.data.loginS === true){
			...
			util.get(this.$baseUrl+'/admin/test').then((result) => {
	...
	methods: {
		...
		async login() {
			...
			if (!loginData.id) return util.showToast('请输入账号', 'none');
			if (!loginData.password) return util.showToast('请输入密码', 'none');
			loginData.password = sm3Digest(loginData.password)
			const result = await util.post(this.$baseUrl+'/login/admin', loginData)
			if (result.status === 200) {
				this.updateAdminInfo(result.entity)
				uni.navigateTo({
					url: 'appointment'
				})
